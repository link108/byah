...
Six posts in, the pieces are: code generation getting cheaper shifts the bottleneck to validation. AI fails in a specific, confident-looking way that doesn't match what most review processes were built to catch. A test suite alone isn't a validation system. CI/CD ends up carrying more of the trust-building work that used to happen in a person's head. Observability picks up the failures that were never anticipated enough to test for. And AI can build most of its own guardrails, as long as something deterministic still enforces them.
